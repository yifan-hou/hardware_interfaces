...
cmd.register(0xB2); -- Velocity-resolved impedance control
...
function process()
    ...
    if id == 0xB2 then
        -- get args
        cmd_pos = bton({payload[2],payload[3],payload[4],payload[5]});
        cmd_force = bton({payload[6],payload[7],payload[8],payload[9]});
        cmd_stiffness = bton({payload[10],payload[11],payload[12],payload[13]});
        cmd_damping = bton({payload[14],payload[15],payload[16],payload[17]});
        ...
        vel_cmd = cmd_pos + (force - cmd_force) / cmd_stiffness - vel * cmd_damping;
