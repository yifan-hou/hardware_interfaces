...
CoinFT::CoinFT(const std::string& port, unsigned int baud_rate, const std::string& calibration_file)
    : io(), serial(io, port), running(false), reading_counter(0), tareFlag(false), tareSampleCount(0), tareSampleTarget(100) {
    ...
    try {
        ...
        std::cout << "Calibration matrix loaded. Dimensions: " << calibrationMatrix.rows() << " x " << calibrationMatrix.cols() << std::endl;
        ...
        try {
            ...
            if (tareFlag.load()) {
                ...
                if (tareSampleCount >= tareSampleTarget) {
                    ...
                    Eigen::VectorXd newTareOffset = sum / static_cast<double>(tareSampleCount);
                    {
            ...
            if (calibrationMatrix.cols() != extendedInput.size()) {
                std::cerr << "Error: Calibration matrix column count (" << calibrationMatrix.cols()
                          << ") does not match extended vector size (" << extendedInput.size() << ")." << std::endl;
                continue;
            }
            ...
            data_cond.notify_one(); // Notify any waiting threads that new data is available
...
std::vector<uint16_t> CoinFT::readRawData() {
    ...
    if (!running) throw std::runtime_error("Data acquisition stopped.");
