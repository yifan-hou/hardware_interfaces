...
#include <practical_socket/PracticalSocket.h> // For UDPSocket and SocketException
...
public:
	...
	int  init(std::chrono::high_resolution_clock::time_point time0, unsigned short portnum, float max_dist_tran, float max_dist_rot, float *safe_zone, EGMSafetyMode mode, bool _print_flag = false, string filefullpath = "");
	...
	/// Set the desired robot cartesian pose.
	/// \param pose dim = 7.
	int SetCartesian(const float *pose);
	...
	EGMSafetyMode _mode; ///< controller mode
	float _max_dist_tran; ///< mm
					      ///< \sa initialization()
	float _max_dist_rot; ///<  rad
							 ///< \sa initialization()
	float *_safe_zone; ///< xmax,xmin,ymax,ymin,zmax,zmin
