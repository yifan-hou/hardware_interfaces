...
bool Realsense::Implementation::initialize(
  ...
  if (config.enable_color && !config.enable_depth) {
    rs_cfg.enable_stream(RS2_STREAM_COLOR, config.width, config.height,
                         RS2_FORMAT_ANY, config.framerate);
  } else if (!config.enable_color && config.enable_depth) {
    rs_cfg.enable_stream(RS2_STREAM_DEPTH, config.width, config.height,
                         RS2_FORMAT_Z16, config.framerate);
  } else if (config.enable_color && config.enable_depth) {
    rs_cfg.enable_stream(RS2_STREAM_ANY, config.width, config.height,
                         RS2_FORMAT_ANY, config.framerate);
  } else {
...
rs2::frameset Realsense::Implementation::next_rgb_frame_blocking() {
  try {
    while (true) {
...
rs2::frameset Realsense::next_rgb_frame_blocking() {
